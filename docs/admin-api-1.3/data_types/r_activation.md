...
|-------|----|-----------|
|**activation** |`xsd:string` | Flag that indicates whether a report suite is active. |
...
|**status** |`xsd:int` | If `in_queue = 0`, then `status = 0`. If `in_queue = 1`, status has one of the following values: `1`: Pending approval to Save. `2`: Pending approval to Delete. **Note:** Populated in method response only. Leave `status` blank for method requests. |
