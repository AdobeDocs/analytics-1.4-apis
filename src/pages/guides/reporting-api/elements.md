...
| hier1-5 |traffic| Hierarchy report. To specify a specific level to report, add a add a `level` and `parentID` parameter to the element structure: ```{	"id":"hier2",	"level":"3", "parentID":"75483925"}``` The `parentID` is obtained from the results of the previous level: ```{ "name": "Home Page", "url": "", "counts": ["72"],   "parentID": "75483925"}``` Not supported by inline segments. |
